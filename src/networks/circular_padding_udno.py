...
class CircularNormUDNO(nn.Module):
    def __init__(
        self,
        in_chans: int,
        out_chans: int,
...
class CircularUDNO(nn.Module):
    ...
    def __init__(
        ...
        self.bottleneck = CircularDISCOBlock(
            ch,
            ch * 2,
            rcut,
            drop_prob,
...
class CircularDISCOBlock(nn.Module):
    """
    ...
    def __init__(
        ...
        self.layers = nn.Sequential(
            CrossCircularPad(pad_size=pad_size),
            DISCO2d(
                in_chans,
                out_chans,
                in_shape=in_shape,
...
class CircularTransposeDISCOBlock(nn.Module):
    """
...
class CrossCircularPad(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        assert self.pad_size < H, "pad_size must be less than height of image (dim -2)"

        top = torch.flip(x[..., :self.pad_size, :], dims=[-1])
        bottom = torch.flip(x[..., -self.pad_size:, :], dims=[-1])
