...
class AAPMRadonTransform(BaseRadonTransform):
    def __init__(self, num_projections):
        super().__init__()
        self.resolution = 256
        self.source_distance = 1075
        self.det_count = 672
        self.thetas = torch.tensor(np.linspace(0.0, 2*np.pi, num_projections, endpoint=False).astype(np.float32))

        self.rf = RadonFanbeam(resolution=self.resolution, angles=self.thetas, source_distance=self.source_distance, det_count=self.det_count)
